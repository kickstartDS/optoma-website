...
Section recipes are also available as an MCP resource (`recipes://section-recipes`) with 19 proven component combinations (14 page-specific, 3 blog-post-specific, 2 event-specific), 14 page templates, and 13 anti-patterns (10 universal, 3 blog-post-specific). All recipes and templates are tagged with a `contentType` — the `list_recipes` tool filters by content type so that e.g. `blog-post` only receives blog-appropriate recipes (text and split focused, no hero or cta sections). Anti-patterns are also filtered by content type.
