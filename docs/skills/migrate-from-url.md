...
- **Andere Content-Typen migrieren:** `contentType` bei `generate_content` und `create_page_with_content` übergeben — z.B. `contentType: "blog-post"` für Blogposts, `contentType: "event-detail"` für Events. Bei Hybrid-Typen (blog-post, blog-overview) zusätzlich `generate_root_field` für Root-Felder (`head`, `aside`, `cta`) und `generate_seo` für SEO-Metadaten aufrufen, dann über `rootFields` an `create_page_with_content` übergeben. Bei Tier-2-Typen (event-detail, event-list) `rootFields` für die Wurzel-Felder setzen.
