...
- **Andere Content-Typen migrieren:** `contentType` bei `generate_content` und `create_page_with_content` übergeben — z.B. `contentType: "blog-post"` für Blogposts, `contentType: "event-detail"` für Events. Bei Tier-2-Typen (event-detail, event-list) zusätzlich `rootFields` für die Wurzel-Felder setzen.
