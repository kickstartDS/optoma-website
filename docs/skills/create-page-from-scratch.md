...
- **Blogpost statt Seite:** `contentType: "blog-post"` bei allen Tools übergeben. Blog-Posts sind Hybrid-Typen mit Sektionen UND Root-Feldern (`head`, `aside`, `cta`, `seo`). Nach den Sektionen `generate_root_field` für jedes Root-Feld und `generate_seo` für SEO-Metadaten aufrufen. Dann `create_page_with_content(sections: [...], rootFields: { head, aside, cta, seo })`. Siehe Skill _„Seite Sektion für Sektion planen und erstellen"_ → Abschnitt Hybrid-Typen.
